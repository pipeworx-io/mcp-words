...
async function pwFetch(url: string | URL, init?: RequestInit): Promise<Response> {
  return fetchWithTimeout(url, init ?? {}, 'Words');
}
  ...
  },
  {
    ...
    inputSchema: {
      ...
      properties: {
        ...
        query: { type: 'string', description: 'Alias for meaning_like.' },
        word: { type: 'string', description: 'Alias for meaning_like.' },
        text: { type: 'string', description: 'Alias for meaning_like.' },
        sounds_like: {
          type: 'string',
...
async function callTool(name: string, args: Record<string, unknown>): Promise<unknown> {
  switch (name) {
    ...
    case 'find_words':
      // When agents reach for find_words with a bare "query"/"word"/"text",
      // they almost always mean meaning_like (semantic search). Map those
      // common forms to meaning_like rather than throwing the schema-strict
      // "at least one of meaning_like/sounds_like/spelled_like" error.
      return findWords(
        (args.meaning_like ?? args.query ?? args.word ?? args.text) as string | undefined,
...
async function findSynonyms(word: string, limit: number) {
  ...
  const res = await pwFetch(`${BASE_URL}/words?${params}`);
  if (!res.ok) throw await httpError(res, 'Datamuse error');
...
async function autocomplete(prefix: string, limit: number) {
  ...
  const res = await pwFetch(`${BASE_URL}/sug?${params}`);
  if (!res.ok) throw await httpError(res, 'Datamuse error');
...
async function findWords(
  ...
  if (!meaningLike && !soundsLike && !spelledLike) {
    // Agents probe with empty args (70+ hits/48h in production analytics);
    // surface concrete example shapes so the retry can succeed without
    // another schema lookup.
    throw new Error(
      'At least one of meaning_like, sounds_like, or spelled_like is required. ' +
      'Try one of: find_words({meaning_like: "ocean"}) for semantic search, ' +
      'find_words({sounds_like: "phone"}) for phonetic matches, ' +
      'or find_words({spelled_like: "b*ttle"}) for spelling patterns. ' +
      'meaning_like also accepts query / word / text as aliases.'
    );
  }
